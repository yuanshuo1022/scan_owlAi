export default new class {
	...
	Type(Original , Method){
		if(this.Confirm(typeof Method , 'object')){
			...
			if(this.In(Method , 'array') && Array.isArray(Original)) return true;
		...
		if(this.Confirm(Method , 'array') && Array.isArray(Original)) return true;
	...
	Confirm(Original , Value){
		return Object.is(Original , Value);
	}
	...
	Negative(Original){
		if(this.Eq(App.Number.Convert(Original) , Math.abs(Original))) return false;
		return true;
	}
	...
	Date(Original){
		...
		if(this.Nan(Date.parse(Original)) && this.Nan((new Date(App.Number.Convert(Original))))) return false;
		return true;
	}
	...
	Wap(){
		...
		App.Each.Of(['android' , 'ipad' , 'iphone' , 'linux' , 'sysmbianos' , 'mobile' , 'windows phone'] , Item => {
			if(Boolean && this.In(App.String.Lower(navigator.userAgent) , Item)) Boolean = false;
			return Boolean;
		});
		return !Boolean;
	}
	...
	Pad(){
		if(!this.Wap() || !this.Lt(screen.width , 1440)) return false;
		return true;
	}
	Wx(){
		return this.In(App.String.Lower(navigator.userAgent) , 'micromessenger');
	}
	Ali(){
		return this.In(App.String.Lower(navigator.userAgent) , 'alipay');
	}
	Transverse(){
		if(this.Wap() && this.In([90 , -90] , orientation)) return true;
		if(this.Gt(screen.width , screen.height)) return true;
		return false;
	}
	Vertical(){
		if(this.Wap() && this.In([0 , 180] , orientation)) return true;
		if(this.Lt(screen.width , screen.height)) return true;
		return false;
	}
}