export default new class {
	...
	#Make(Option){
		if(App.Verify.Array(Option)){
			...
			const Response = {};
			Response.url = App.Check(Option.Url , 'String' , location.origin);
	...
	Stream(Original){
		...
		Option.Link = App.Check(Option.Link , 'Require' , location.hostname , 'string');
		...
		const Resource = new EventSource(Option.Link);
	...
	Socket(Original){
		...
		Original.Url = App.Check(Original.Url , 'Require' , location.hostname , 'string');
		if(App.Verify.Confirm('/' , App.String.Substr(Original.Url , 0 , 1))) Original.Url = App.String.Concat(location.hostname , Original.Url);
		...
		Original.Option.Function = new class {
			Connect(){
				...
				Original.Option.Connect = new WebSocket(Original.Url);
